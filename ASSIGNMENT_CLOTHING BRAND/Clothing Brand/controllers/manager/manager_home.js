...
router.get('/noticeinfo', (req, res)=>{
	...
	noticeinfoModel.getAll(function(results){
		console,console.log(results);
		var uname = req.cookies['uname'];
		res.render('manager_home/noticeinfo', {noticeinfoList: results, uname});
	});
...
router.get('/pdf', (req, res)=>{
	...
	pdf.create(html, options).toFile('assets/uploads/noticeinfoList.pdf', function (err, res) {
        if (err) { return console.log(err); }
        else {
            console.log(res); // { filename: '/app/businesscard.pdf' } 
            // var datafile = fs.readFileSync('assets/uploads/invoice.pdf');
